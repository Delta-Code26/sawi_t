...
if (isset($_POST['search'])) {
    ...
    $pekerja_data = searchData($conn, 'tb_pekerja', 'nama_lengkap', $keyword);
} else {
    // Fetch all data from tb_pekerja
    $pekerja_data = fetchAll($conn, 'tb_pekerja');
}
...
if (isset($_POST['submit'])) {
    ...
    if (move_uploaded_file($tmp, $foto_path)) {
        ...
        $stmt->bind_param("ssssssssss", $id_pekerja, $username, $hashed_password, $nama_lengkap, $jk, $email, $status, $tgl_gabung, $grade, $foto);
            ...
            <tbody>
                <?php foreach ($pekerja_data as $index => $pekerja) : ?>
                    <tr class="text-center">
                        ...
                        <td><?php echo htmlspecialchars($pekerja['id_pekerja']); ?></td>
                        ...
                        <td>
                            <?php if (!empty($pekerja['foto'])) : ?>
                                <img src="./assets/uploads/<?php echo htmlspecialchars($pekerja['foto']); ?>" alt="Foto" class="circle" style="width: 40px; height: 40px;">
                            <?php else : ?>
                                Tidak ada foto
                            <?php endif; ?>
                        </td>
                        <td>
                            <!-- <a href="?page=update_pekerja&id=<?php echo htmlspecialchars($pekerja['id_pekerja']); ?>" title="Edit pekerja" class="btn btn-success">
                                <i class="far fa-edit"></i>
                            </a>
                            <a href="?page=hapus_pekerja&id=<?php echo htmlspecialchars($pekerja['id_pekerja']); ?>" onclick="return confirm('Apakah anda yakin ingin menghapus pekerja ini ?')" title="Hapus Buku" class="btn btn-danger">
                                <i class="fas fa-trash-alt"></i>
                            </a> -->
                            <a href="?page=update_pekerja&kode=<?php echo $pekerja['id_pekerja']; ?>" title="Edit pekerja" class="btn btn-success">
                                <i class="far fa-edit"></i>
                            </a>
                            <a href="?page=del_pekerja&kode=<?php echo $pekerja['id_pekerja']; ?>" onclick="return confirm('Apakah anda yakin ingin menghapus admin ini ?')" title="Hapus Data" class="btn btn-danger">
                                <i class="fas fa-trash-alt"></i>
                            </a>
                        </td>
                    </tr>
