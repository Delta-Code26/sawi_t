...
$cari_id = mysqli_query($conn, "SELECT id_user FROM users WHERE level = 'Admin'");
if (mysqli_num_rows($cari_id) > 0) {
    $data_id = mysqli_fetch_array($cari_id);
    $kode = $data_id['id_user'];
    $urut = substr($kode, 1); // Mengambil bagian nomor urut tanpa huruf "P"
    $tambah = (int) $urut + 1;
    $format = "A" . str_pad($tambah, 6, "0", STR_PAD_LEFT); // Menghasilkan format ID dengan 9 angka
} else {
    $format = "A000001"; // Format default jika tidak ada data pekerja
}
...
if (isset($_POST['submit'])) {
    ...
    if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($tmp, $foto_path)) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (id_user, username, password, nama_lengkap, jk, email, status, tgl_gabung, grade, foto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssssssss", $id_user, $username, $hashed_password, $nama_lengkap, $jk, $email, $status, $tgl_gabung, $grade, $foto_name);
